...
import { readTasksFromFirebaseDb, updateTaskDataByPath, writeTasksToFirebaseDb } from 'src/mixins/firebase';
import { defineStore } from 'pinia';
import { Task } from 'src/types';
// import { readFromDbSync, saveAll } from 'src/mixins/jsondb.js';
import { getFromLocalStorage, getStoriesFromTask, localStorageNames, queueTaskRefresh } from 'src/utils';
...
const useTaskStore = defineStore('taskStore', {
                ...
                handler: (task, extra) =>
                {
                    return !(getStoriesFromTask(task).length) && (
                        (!extra || !extra.tags) ? true : (
                            task.tags || []).some((tag) => extra.tags && extra.tags.includes(tag)
                        )
                    );
                }
            },
  ...
  getters: {
    ...
    actions: {
        ...
        async cloudUpdateSingleProperty({ taskId, prop, data }: {
            taskId: Task['id'];
            prop: string;
            data: any;
        })
        ...
        SET_CATEGORIES()
        {
            ...
            if (storedCategories)
            {
                ...
                {
                    const ctg = storedCategories.find((c: TaskBucket) => c.title === def.title);
